...
<html>
    ...
    <body>
        <div class="container col-md-12">
            ...
            <div class="container">
                <h1>Sighting Added!</h1>
                <h3>Date: ${date}</h3>
                <h3>Time: ${time}</h3>
                <h3>Heroes: 
                    <c:forEach var="hero" varStatus="status" items="${sighting.heroes}">
                        <c:out value="${hero.name}"/>
                        ${not status.last ?'| ' : ''}
                    </c:forEach>
                </h3>
                <h3>Location: "${sighting.location.name}"
                </h3>
                <a href="${pageContext.request.contextPath}/sightings">
                    <button class="btn btn-default" name="sightingButton">
                        Return to Sightings</button>
                </a>
            </div>
