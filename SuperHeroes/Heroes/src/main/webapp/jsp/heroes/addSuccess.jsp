    ...
    <body>
        <div class="container col-md-12">
            ...
            <div class="container">
                ...
                <h3>List of Powers: 
                    <c:forEach var="power" varStatus="status" items="${hero.powers}">
                        <c:out value="${power.name}"/>
                        ${not status.last ?', ' : ''}
                    </c:forEach>
                </h3>
                <h3>Organization Affiliations: 
                    <c:forEach var="org" varStatus="status" items="${orgs}">
                        <c:out value="${org.name}"/>
                        ${not status.last ?', ' : ''}
                    </c:forEach>
                </h3>
                <a href="${pageContext.request.contextPath}/heroes">
                    <button class="btn btn-default" name="heroButton">
                        Return to Heroes</button>
                </a>
            </div>
